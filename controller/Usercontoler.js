...
export const register=async(req,res)=>{
  ...
  //console.log(uplod)
       const {name,nid,password,role,email}=req.body

             if(!name || !email || !nid || !password || !role){
             return res.status(200).json({msg:"missing"})
             }
   ...
   const create=new User({
      name,email,nid, password:await bcrypt.hash(password,10),role
   })
   console.log(create)
   await create.save()
...
export const login=async(req,res)=>{
    ...
    try {
        ...
        const nidcardchek=await User.findOne({email})
...
try {
    ...
    return res.status(200).json({msg:"me",user})
    console.log(user)
